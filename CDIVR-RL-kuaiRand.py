...
def main(args):
    ...
    if hasattr(user_model, 'ab_embedding_dict') and args.is_ab:
        ...
        alpha_u = np.ones([27285, 1])
        beta_i = np.ones([7583, 1])
    ...
    register(
        id=args.env,  # 'KuaiRand-v0',
        entry_point='environments.KuaiRand_Pure.env.KuaiRand:KuaiRandEnv',
        kwargs = {"yname": args.yfeat,
                     "mat": mat,
    ...
    # 测试集的用户(10000)对所有视频(7583)的偏好已提前计算好
    ...
    policy = PPOPolicy(
        ...
        action_bound_method="" if args.env == "KuaiRand-v0" else "clip",
        action_scaling=False if args.env == "KuaiRand-v0" else True
    )
    ...
    policy.callbacks = [
        Callback_Coverage_Count(test_collector_set, df_item_val, need_transform=False,
                                item_feat_domination=item_feat_domination,
                                lbe_photo=None, top_rate=args.top_rate),
