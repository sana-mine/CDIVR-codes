...
class KuaishouEnv(gym.Env):
    ...
    def __init__(self, mat=None, lbe_user=None, lbe_photo=None, list_feat=None, df_item_val=None, df_dist_small=None,
        ...
        if mat is not None:
            ...
            self.mat, self.lbe_user, self.lbe_photo, self.list_feat, self.df_item_val, self.df_dist_small = self.load_mat()
    ...
    def load_mat():
        ...
        df_item_val = df_feat.loc[photo_list]
        df_item_val['photo_duration'] = np.array(
            list(map(lambda x: photo_mean_duration[x], df_item_val.index)))
        ...
        return mat, lbe_user, lbe_photo, list_feat, df_item_val, df_dist_small

    @staticmethod
    def compute_normed_reward(user_model, lbe_user, lbe_photo, df_item_val, lambda_var=1):
        ...
        item_info = df_item_val.loc[lbe_photo.classes_]
        ...
        for i, user in tqdm(enumerate(lbe_user.classes_), total=n_user, desc="predict all users' rewards on all items"):
            ...
            reward = user_model.forward(ui)
            reward_u = reward.detach().squeeze().cpu().numpy()
            #var_u = var.detach().squeeze().cpu().numpy()
